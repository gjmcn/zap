...
test('$binCount numeric array default', () => {
  expect(compute('@ 5 9 12 7 $binCount (@ 10 20)'))
...
test('$bin array-of-arrays', () => {
  expect(compute(
    `${xStr} ${limitsStr} x $bin limits [a :0 - b]`
...
test('$bin array-of-arrays, second callback', () => {
  expect(compute(
    `${xStr} ${limitsStr} x $bin limits [a :0 - b] [a $map [a :1]]`
  )).toStrictEqual(new Map([
    [10, [1, 4]], 
    [20, []],
    [30, [0]],
...
test('$binCount array-of-arrays', () => {
  expect(compute(
    `${xStr} ${limitsStr} x $binCount limits [a :0 - b]`
  )).toStrictEqual(new Map([
    [10, 2], 
    [20, 0],
    [30, 1],
...
test('$binCount array-of-arrays, second callback', () => {
  expect(compute(
    `${xStr} ${limitsStr} x $binCount limits [a :0 - b] [a > 1]`
  )).toStrictEqual(new Map([
    [10, true], 
    [20, false],
    [30, false],
