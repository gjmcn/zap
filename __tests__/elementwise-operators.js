...
testEach('non-string iterables', [
  ...
  ["$round (2.4 >> 4.8 1.2)",               [2, 4, 5]],
  ["@ 0 'a' $boolean",                      [false, true]],
  ['\\[** $yield false; $yield 5] $string', ['false', '5']],
  ['$number (@)',                           []],
], true);

// string operators throw on undefined or null
test('string operator, throw on undefined', () => {
  expect(() => compute('undefined $toUpperCase')).toThrow();
});
test('string operator, throw on null', () => {
  expect(() => compute('@ " a" null $trim')).toThrow();
});